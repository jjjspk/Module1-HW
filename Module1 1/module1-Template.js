...
class GameStore {
  ...
  //     addGame(game: Game): จาก class Game
  // • Parameters: game type: game object
  // • Adds a new Game object to the inventory. Ensure the game has valid
  ...
  purchaseGame(id) {
    const purchase = this.inventory.findIndex((game) => game.id === id); //id ตรงมั้ย

    this.inventory.pop(purchase); //ถ้าตรงก็ลบออก
  }
  ...
  updateGamePrice(id, newPrice) {
    const gameUpdate = this.inventory.find((game) => game.id === id); //id ตรงมั้ย
    if (gameUpdate) {
  ...
  totalInventoryValue() {
    return this.inventory.reduce((total, game) => total + game.price, 0); //คำนวณบวกราคาของเกม
  }
  listGenres() {
